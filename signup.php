...
<body>
    <div>
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
            ...
            <?php
            if ($post && !$empty_fields && !$unmatch) {
                echo '<span>Username can contain alphabet letters, numbers, and underscore(_), but must begin with a letter. It must be at least 4 characters long.<br></span>';
            }
            if (!empty($uname_err) && $uname_err) {
                echo '<span>Username taken. Try another username.</span>';
            }
            ?>

            <br>Name<br>
            <input type="text" name="fname" value="<?php echo isset($_POST['fname']) ? $_POST['fname'] : ''; ?>" placeholder="First Name"><br>
            <input type="text" name="lname" value="<?php echo isset($_POST['lname']) ? $_POST['lname'] : ''; ?>" placeholder="Last Name"><br>

            <?php
            if ($post && !$empty_fields && !($lnmatch && $fnmatch)) {
                echo '<span>Name can only contain alphabet letters.<br></span>';
            }
            ?>

            <br>E-mail<br>
            <input type="text" name="email" value="<?php echo isset($_POST['email']) ? $_POST['email'] : ''; ?>" placeholder="email@example.com"><br>

            <?php
            if (!empty($email_err) && $email_err) {
                echo '<span>Email already registered. Enter another email.</span>';
            }
            if ($post && !$empty_fields && !$emmatch) {
                echo '<span>Email must be of the format example@site.domain<br></span>';
            }
            ?>
            ...
            <?php
            if ($post && !$empty_fields && !$pmatch) {
                echo '<span>Password must be at least 5 characters long</span>';
            }
            if ($post && $empty_fields) {
                echo "<br><span>Please fill all the fields completely.</span><br>";
            }
            if ($post && $pmatch && !$peq) {
                echo '<span>Passwords don\'t match</span><br>';
            }
            ?>
